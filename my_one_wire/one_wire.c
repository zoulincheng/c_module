...
PROCESS(ds18b20, "ds18");	//for test 

DS18B20_DEVICE tg_ds18b20 = {0};
//--------------------------------------------------------------------------
...
int OWSearch(void)
{
	...
	{
		LastDiscrepancy = 0;LastDeviceFlag = FALSE;
		LastFamilyDiscrepancy = 0;
		search_result = FALSE;
	}
...
void OWWriteByte(unsigned char byte_value)
{
	...
	//TODO: impl
	one_wire_write_byte(byte_value);
}
//--------------------------------------------------------------------------
...
void ds18b20_convert_temperature(u_char *pid) {
	one_wire_reset_pulse();
	one_wire_match_rom(pid); // Match ROM
	one_wire_write_byte(0x44); // Convert temperature
}

uint8_t one_wire_crc(uint8_t data) {
    crc8 = dscrc_table[crc8 ^ data];
    return crc8;
}

short ds18b20_decode_temperature(void) {
	int i;
	u8 crc;
	u8 data[9];
	short temp;
...
short ds18b20_read_temperature(u_char *pid) {
	one_wire_reset_pulse();
	one_wire_match_rom(pid); // Match ROM
...
void ds18b20_find(DS18B20_DEVICE *pdev)
{
	int rslt = 0;
	int i = 0;
	int cnt = 0;
	...
	while (rslt)
	{
		...
		for (i = 0; i <= 7; i++)
		{
			xprintf("%02X", ROM_NO[i]);
			
		}
		*/
		memcpy(pdev->id[cnt++], ROM_NO, 8);
		pdev->cnt = cnt;
		//xprintf(" %d\n",cnt);
		rslt = OWNext();
	}
	for (i = 0; i < pdev->cnt; i++)
	{
		MEM_DUMP(0, "ID", pdev->id[i], 8);
	}
	cnt = 0;
}

void ds18b20_read_temp(DS18B20_DEVICE *pdev)
{
	int i = 0;
	short temp;
	...
	if (pdev->cnt)
	{
		for (i = 0; i < pdev->cnt; i++)
		{
			temp = ds18b20_read_temperature(pdev->id[i]);
			XPRINTF((12, "TEMP = %04x\r\n", temp)); 
			//short nTemper  = ((long)nValue*625)/100;
			pdev->ptemp = ((long)temp*625)/100;
			XPRINTF((10, "TEMPd = %d.%d\r\n", pdev->ptemp/100,  pdev->ptemp%100)); 
		}
	}
}
