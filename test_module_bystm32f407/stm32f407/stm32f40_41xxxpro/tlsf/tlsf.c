...
pool_t tlsf_add_pool(tlsf_t tlsf, void *mem, size_t bytes)
{
    ...
    //xprintf("bytes = %d, pool_overhead=%d   bytes - pool_overhead =%d\r\n", bytes - pool_overhead);

    if (((ptrdiff_t)mem % ALIGN_SIZE) != 0) {
        xprintf("tlsf_add_pool: Memory must be aligned by %u bytes.\n",
    ...
    if (pool_bytes < block_size_min || pool_bytes > block_size_max) {
        xprintf(
            "tlsf_add_pool: Memory size must be between %u and %u bytes. pool_overhead = %d pool_bytes=%u\n",
            pool_overhead + block_size_min, pool_overhead + block_size_max, pool_overhead, pool_bytes);
        return 0;
    }
...
{
    if (((ptrdiff_t)mem % ALIGN_SIZE) != 0) {
        xprintf("tlsf_create: Memory must be aligned to %u bytes.\n",
               (unsigned int)ALIGN_SIZE);
        return NULL;
    }
...
tlsf_t tlsf_create_with_pool(void *mem, size_t bytes)
{
    ...
    xprintf("mem = %08x, tlsf_size()=%d, bytes=%d, bytes-tlsf_size=%d\r\n", mem, tlsf_size( ), bytes, bytes-tlsf_size( ));
    return tlsf;
}
